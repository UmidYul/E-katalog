  ...
  return (
    <div className="space-y-3">
      <div className="relative aspect-square overflow-hidden rounded-2xl border border-border bg-card">
        <Image src={active} alt="Product image" fill className="object-cover" sizes="(max-width: 768px) 100vw, 40vw" />
      </div>
      <div className="grid grid-cols-5 gap-2">
        {list.slice(0, 5).map((src) => (
          <button key={src} className={`relative aspect-square overflow-hidden rounded-xl border ${active === src ? "border-primary" : "border-border"}`} onClick={() => setActive(src)}>
            <Image src={src} alt="Product thumbnail" fill className="object-cover" sizes="120px" />
          </button>
        ))}
      </div>
    </div>

...
def test_cleanup_auth_legacy_redis_keys_task_routing() -> None:
    route = celery_app.conf.task_routes.get("app.tasks.maintenance_tasks.cleanup_auth_legacy_redis_keys")
    assert route == {"queue": "maintenance", "routing_key": "maintenance"}


def test_cleanup_auth_legacy_redis_keys_schedule_registered() -> None:
    schedule = celery_app.conf.beat_schedule.get("cleanup-auth-legacy-redis-keys-daily-0420")
    assert schedule is not None
    assert schedule["task"] == "app.tasks.maintenance_tasks.enqueue_cleanup_auth_legacy_redis_keys"
